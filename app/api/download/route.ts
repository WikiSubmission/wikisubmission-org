...
function stripMetadata(buffer: ArrayBuffer, contentType: string): ArrayBuffer {
    ...
    if (uint8.length > 128) {
        ...
        if (uint8[tagOffset] === 0x54 && uint8[tagOffset + 1] === 0x41 && uint8[tagOffset + 2] === 0x47) {
            end = tagOffset;
        }
    }
    ...
    if (type.includes('mpeg') || type.includes('mp3')) {
        ...
        for (let i = start; i < searchLimit; i++) {
            if (uint8[i] === 0x66 && uint8[i + 1] === 0x4C && uint8[i + 2] === 0x61 && uint8[i + 3] === 0x43) {
                start = i;
                break;
            }
        }
        ...
        const searchLimit = Math.min(start + 4096, end - 4);
        for (let i = start; i < searchLimit; i++) {
            if (uint8[i] === 0x52 && uint8[i + 1] === 0x49 && uint8[i + 2] === 0x46 && uint8[i + 3] === 0x46) {
                start = i;
                break;
            }
        }
