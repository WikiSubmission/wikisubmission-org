...
export default function ContributorLayout({ children }: { children: React.ReactNode }) {
    ...
    if (!user?.organizationMemberships
        .find((membership) => membership.organization.name === "Contributor")
    ) {
        return !user ? (
            <div className="text-xs p-4 bg-muted/50 rounded-2xl gap-1 flex items-center justify-center w-fit text-muted-foreground">
                ...
                <p>You must be <Link href="/auth/sign-in?redirectUrl=/dashboard/contributor/overview" className="text-violet-600 hover:text-violet-500 cursor-pointer">signed in</Link> to view this page.</p>
            </div>
        ) : (
            <div className="text-xs p-4 bg-muted/50 rounded-2xl gap-1 flex items-center justify-center w-fit text-muted-foreground">
                <AlertCircle className="size-4 text-violet-600" />
