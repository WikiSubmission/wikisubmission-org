  ...
  return (
    <main className="space-y-4 max-w-full overflow-hidden">
      ...
      <section className="flex flex-wrap gap-2 items-center justify-between">
        <div className="flex flex-wrap gap-2">
          <section className="p-4 bg-muted/50 rounded-2xl">
            <p className="text-4xl font-semibold">{tracks.length}</p>
            ...
            <p className="text-4xl font-semibold">
              {
                tracks.filter(
                  (i) =>
                    i.release_date &&
                    new Date(i.release_date).getTime() >
                      new Date().getTime() - 60 * 60 * 24 * 7 * 1000
                ).length
              }
            </p>
            <p className="text-muted-foreground">release(s) this week</p>
          </section>
        </div>
      </section>
      ...
      <section className="space-y-2">
        <TrackDialog artists={artists} categories={categories} />

        <div className="space-y-2">
          {tracks.map((track) => (
            <section
              key={track.id}
              className="p-4 bg-muted/50 rounded-2xl flex justify-between items-center"
            >
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  <p
                    className="text-[10px] text-center text-muted-foreground hover:text-violet-500 cursor-pointer py-1 px-2 rounded-full bg-muted/50 w-fit flex items-center gap-1"
                    onClick={() => {
                  ...
                  <p
                    ...
                    onClick={() => {
                      navigator.clipboard.writeText(
                        `https://wikisubmission.org/music?track=${track.id}`
                      )
                      toast.success('Link copied to clipboard')
                    }}
                  >
...
function TrackDialog({
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      const artistName =
        artists.find((a) => a.id === artistId)?.name || 'Unknown'
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? 'Edit' : 'New'} Track • Step {step} of 2
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed break-words">
                Uploads will be stored at our S3 bucket (
                <code className="bg-muted px-1 rounded break-all">
                  wikisubmission
                </code>
                ) at path:{' '}
                <code className="bg-muted px-1 rounded break-all">
                  media/zikr/&#123;Artist&#125;/&#123;Title&#125;
                </code>
              </p>
            </div>
          </div>
