...
export default function NotificationsPage() {
  ...
  const isInactive = (
    notification: Database['internal']['Tables']['ws_notifications_ios']['Row']
  ) => {
    const prayerTimesOff =
      !notification.prayer_times_notifications ||
  ...
  const displayNotifications = notifications
    .filter((notification) =>
      filterInactive ? isInactive(notification) : true
    )
    .sort((a, b) => {
      if (sortBy === 'last_delivery') {
        const aTime = a.last_delivery_at
          ? new Date(a.last_delivery_at).getTime()
          : 0
        const bTime = b.last_delivery_at
          ? new Date(b.last_delivery_at).getTime()
          : 0
    ...
    <main className="space-y-4 max-w-full overflow-hidden">
      ...
      <section className="flex flex-wrap gap-2">
        <section className="p-4 bg-muted/50 rounded-2xl">
          ...
          <p className="text-muted-foreground">registered devices</p>
        </section>
          ...
          <p className="text-4xl font-semibold">
            {
              notifications.filter(
                (i) =>
                  i.last_delivery_at &&
                  new Date(i.last_delivery_at).getTime() >
                    new Date().getTime() - 60 * 60 * 1000
              ).length
            }
            +
          </p>
          <p className="text-muted-foreground">deliveries in last hour</p>
        </section>
      </section>
      ...
      <section className="flex flex-wrap gap-2 items-center">
        ...
        <span className="text-[10px] uppercase tracking-wider text-muted-foreground ml-2 mr-1">
          Filter
        </span>
        <button
      ...
      <section className="space-y-2">
        {displayNotifications.map((notification) => (
          <div
            key={notification.device_token}
            className="p-4 bg-muted/50 rounded-2xl w-full min-w-0 text-xs space-y-2"
          >
            <section className="flex items-start w-full justify-between">
              <div className="flex gap-2 items-center bg-muted/50 p-2 rounded-full">
                ...
                <p className={`font-semibold ${Fonts.geistMono.className}`}>
                  {notification.device_token.slice(0, 5) +
                    '...' +
                    notification.device_token.slice(-5)}
                </p>
              </div>
            ...
            <section className="space-y-1">
              ...
              notification.prayer_times_notifications.enabled ? (
                <div className="flex gap-2">
                  ...
                  {'location' in notification.prayer_times_notifications &&
                  typeof notification.prayer_times_notifications.location ===
                    'string' ? (
                    <div>
                      <p className="text-muted-foreground flex items-center gap-1 break-all">
                        <MapPinIcon className="size-3 text-amber-500 shrink-0" />
                        {notification.prayer_times_notifications.location
                          .split(',')
                          .join(', ')}
                      </p>
                    </div>
              ...
              {notification.daily_verse_notifications &&
              typeof notification.daily_verse_notifications === 'boolean' &&
              notification.daily_verse_notifications ? (
              ...
              {notification.daily_chapter_notifications &&
              typeof notification.daily_chapter_notifications === 'boolean' &&
              notification.daily_chapter_notifications ? (
                <div className="space-y-1">
                  <p className="text-muted-foreground flex items-center gap-1">
                    <CheckIcon className="size-3 text-violet-600" />
              ...
              {notification.announcement_notifications &&
              typeof notification.announcement_notifications === 'boolean' &&
              notification.announcement_notifications ? (
                <div className="space-y-1">
                  <p className="text-muted-foreground flex items-center gap-1">
                    <CheckIcon className="size-3 text-violet-600" />
            ...
            <div>
              <SendNotificationDialog deviceToken={notification.device_token} />
            </div>
          </div>
        ))}
      </section>
...
function SendNotificationDialog({ deviceToken }: { deviceToken: string }) {
  const queryClient = useQueryClient()
  const [apiKey, setApiKey] = useLocalStorage('notification_api_key', '')
  const [type, setType] = useState<
    'custom' | 'random_verse' | 'prayer_times' | 'daily_verse' | 'daily_chapter'
  >('random_verse')
          ...
          <div className="space-y-2">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                This key is required to send notifications. It is defined in the
                push-notifications server&apos;s{' '}
                <a
                ...
                as the{' '}
                <span
                  className={`${Fonts.geistMono.className} bg-muted/50 px-1 rounded`}
                >
                  API_KEY
                </span>{' '}
          ...
          <div className="space-y-4 border-t pt-4">
            <div className="flex items-start space-x-2">
              ...
              <div className="grid gap-1.5 leading-none">
                <Label htmlFor="force">Force delivery</Label>
