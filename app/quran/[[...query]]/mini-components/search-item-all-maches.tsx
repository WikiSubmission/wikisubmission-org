...
export function SearchItemAllMatches({
  props,
}: {
  props: {
    results: SearchHit | SearchHitText | SearchHitSubtitle | SearchHitFootnote
  }
}) {
  ...
  if (props.results.hit === 'text' && quranPreferences.text) {
    return (
      <div className="space-y-2 bg-muted/50 rounded-2xl p-4">
        ...
        >
          <p>TEXT - {props.results.verse_id}</p>
          <ArrowUpRightIcon className="size-4" />
        </Link>

        <section>
          <p>
            <strong>[{props.results.verse_id}]</strong>{' '}
        ...
        {quranPreferences.secondaryLanguage && (
          <section>
            <p>
              <strong>[{props.results.verse_id}]</strong>{' '}
              {highlightMarkdown(
                props.results[quranPreferences.secondaryLanguage]
              )}
            </p>
          </section>
        )}
    ...
    return (
      <div className="space-y-2 bg-muted/50 rounded-2xl p-4">
        <Link
          ...
          className="flex items-center gap-1 w-fit text-xs text-muted-foreground tracking-wider text-violet-600 hover:cursor-pointer"
        >
          <p>SUBTITLE - {props.results.verse_id}</p>
          <ArrowUpRightIcon className="size-4" />
        </Link>
        <section>
          <p>{highlightMarkdown(props.results.english)}</p>
        </section>
      </div>
    )
    ...
    return (
      <div className="space-y-2 bg-muted/50 rounded-2xl p-4 text-muted-foreground">
        <Link
          ...
          className="flex items-center gap-1 w-fit text-xs text-muted-foreground tracking-wider text-violet-600 hover:cursor-pointer"
        >
          <p>FOOTNOTE - {props.results.verse_id}</p>
          <ArrowUpRightIcon className="size-4" />
        </Link>
        <section>
