...
export function SearchItemChapter({ chapter }: { chapter: SearchHitChapter }) {
  ...
  return (
    <Link href={`/quran/${chapter.chapter_number}`}>
      <div className="flex items-center gap-1 bg-muted/50 p-4 rounded-2xl w-fit hover:bg-muted/80">
        <p>
          <strong>Sura {chapter.chapter_number}:</strong>{' '}
          {highlightMarkdown(
            chapter[`title_${quranPreferences.primaryLanguage}`]
          )}{' '}
          ({highlightMarkdown(chapter.title_transliterated)})
        </p>
