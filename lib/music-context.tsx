...
export function MusicProvider({
  ...
  const [playbackContext, setPlaybackContext] =
    useState<PlaybackContextType>('allTracks')
  const [queue, setQueue] = useState<UnifiedTrack[]>([])
  const [favorites, setFavorites] = useLocalStorage<string[]>(
    'zikr_favorited_tracks',
    []
  )
  ...
  useEffect(() => {
    ...
    const artworkUrl =
      currentTrack.artist.image_url || '/brand-assets/logo-black.png'
  ...
  const playTrack = (
    ...
    if (context) {
      ...
      if (customQueue) {
        ...
        switch (context) {
          case 'category':
            newQueue = allTracks.filter(
              (t) => t.category.id === track.category.id
            )
            break
          case 'favorites':
            newQueue = allTracks.filter((t) => favorites.includes(t.url))
            break
          case 'allTracks':
          default:
            newQueue = allTracks
  ...
  const toggleFavorite = (trackUrl: string) => {
    setFavorites((prev: string[]) => {
      if (prev.includes(trackUrl))
        return prev.filter((u: string) => u !== trackUrl)
